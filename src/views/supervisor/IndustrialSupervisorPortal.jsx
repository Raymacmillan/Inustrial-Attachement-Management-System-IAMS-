  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await supervisorService.approveWeek(
        weekId,
        user.id,
        supervisor?.name || user.email,
        supervisor?.title || "Industrial Supervisor"
      );
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gray-100 pb-8">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            Welcome back, {supervisor.name || user.email}
          </p>
        </div>
                  ...
                  {s.weeks.map((w) => (
                    <button
                      ...
                      className={`w-7 h-7 rounded-lg text-[10px] font-black transition-all
                        ...
                        : w.status === "action_needed" ? "bg-red-500    text-white cursor-pointer hover:bg-red-600"
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {selectedStudent.weeks.map((w) => {
              ...
              const canReview  = isPending || isApproved || isFlagged;
              ...
              return (
                <div key={w.id} className={`bg-white rounded-2xl border-2 p-5 transition-all
                  ...
                  {canReview ? (
                    ...
                    >
                      <BookOpen size={13} />
                      {isPending ? "Review Logbook" : isApproved ? "View Entry" : "View Feedback"}
                    </Button>
                  ) : (
