...
function ManualAllocateRow({ student, vacancies, onAllocate, processing }) {
  ...
  return (
    <div className="bg-white border-2 border-dashed border-gray-200 hover:border-brand-200 rounded-2xl p-5 transition-all">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
              {student.doc_warning && (
                <span className="text-[9px] text-amber-600 font-black uppercase flex items-center gap-1">
                  <FileWarning size={10} /> Docs Missing
                </span>
              )}
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            <select
              className="w-full bg-gray-50 border border-gray-200 rounded-xl text-xs font-semibold px-3 py-2.5 pr-8 focus:ring-2 focus:ring-brand-500 outline-none appearance-none cursor-pointer"
          ...
          <Button
            size="sm"
            disabled={!selectedVacancy || processing}
            loading={processing}
...
export default function MatchingEngine() {
  ...
  // ── Derived state ──
  // Edge function already returns ONE entry per student (best vacancy match).
  ...
  const allSelected         = placeableMatches.length > 0 && selectedIds.size === placeableMatches.length;

  // Unique vacancies available for the manual picker
  ...
  // stopPropagation prevents the card's onClick from firing when
  // the Select All button is clicked — without this the first student
  // gets toggled immediately after select-all runs, appearing to do nothing
  const toggleAll = (e) => {
    e.stopPropagation();
    setSelectedIds(
      allSelected
        ? new Set()
        : new Set(placeableMatches.map(s => s.student_id))
    );
  };
  ...
  const allocateOne = async (match) => {
    ...
    // 2. Check vacancy still has slots — guards against race conditions in batch
    ...
    if (!vacancy || vacancy.available_slots < 1) {
      throw new Error(`No slots remaining for "${match.role}" at ${match.org_name}. Vacancy is full.`);
    }

    // 3. Fetch org contact details to auto-populate the industrial supervisor.
    //    The coordinator can override these later in the Student Audit Modal.
    const { data: org } = await supabase
      .from("organization_profiles")
      .select("contact_person, supervisor_email")
      .eq("id", match.org_id)
      .single();

    // 4. Create the placement, seeding industrial supervisor from org profile
    const { error: pError } = await supabase.from("placements").insert({
      ...
      duration_weeks:              8,
      // Industrial supervisor auto-filled from org — coordinator can update later
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="relative z-10 space-y-4">
          <div>
            ...
            <p className="text-brand-200 text-sm max-w-lg mt-1 leading-relaxed">
              Run the engine, check students for auto-allocation, then handle the rest manually — all from one screen.
            </p>
          </div>
      ...
      {placeableMatches.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <div>
              ...
              <p className="text-[10px] text-gray-400 mt-0.5">
                Check students to auto-allocate. Unchecked students move to the manual queue.
              </p>
            </div>
            {/* stopPropagation is critical here — prevents card toggle firing */}
            <button
              onClick={toggleAll}
          ...
          {placeableMatches.map((match, idx) => {
            const isChecked = selectedIds.has(match.student_id);
            return (
              <div
                key={match.student_id}
                onClick={() => toggleSelect(match.student_id)}
                className={`bg-white border-2 rounded-3xl p-5 cursor-pointer transition-all select-none ${
                  isChecked ? "border-brand-500 bg-brand-50/20 shadow-md" : "border-gray-100 hover:border-gray-200"
                }`}
              >
                <div className="flex items-start gap-4">

                  {/* Checkbox — stopPropagation so clicking it doesn't double-fire with card */}
                  <div
                    ...
                    onClick={e => { e.stopPropagation(); toggleSelect(match.student_id); }}
                  >
                  ...
                  <div className="relative shrink-0">
                    <div className="w-14 h-14 bg-brand-50 rounded-2xl flex items-center justify-center text-brand-600 font-black text-base">
                      {match.total_score}%
                    </div>
                  ...
                  <div className="flex-1 min-w-0 space-y-1.5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {[["Skills", match.score_breakdown?.skills], ["GPA", match.score_breakdown?.gpa], ["Location", match.score_breakdown?.location]].map(([label, pts]) => (
                        <span key={label} className="text-[9px] px-2 py-0.5 bg-brand-50 text-brand-600 rounded-full font-black uppercase">
                          {label} {pts || 0}pts
                        </span>
                      ))}
                    </div>
                    ...
                    {match.doc_warning && (
                      <div className="flex items-center gap-1.5 text-[10px] text-amber-600 font-bold">
                        <FileWarning size={11} className="shrink-0" /> {match.doc_warning}
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 px-1">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 mt-0.5">
                Pick a vacancy and allocate directly. Or check them above to include in auto-allocation.
              </p>
            </div>
          </div>
          {manualQueue.map((student, idx) => (
            <ManualAllocateRow
