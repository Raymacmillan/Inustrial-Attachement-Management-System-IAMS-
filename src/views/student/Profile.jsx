...
export default function StudentProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ type: "success", text: "Academic profile updated!" });
    ...
    <div className="w-full max-w-5xl mx-auto px-4 py-8 space-y-8 animate-in fade-in duration-500 pb-24">
      ...
      <header className="flex flex-col sm:flex-row items-center gap-6 border-b border-gray-100 pb-8">
        ...
        <div className="flex-1 min-w-0 text-center sm:text-left">
          <div className="hero-tag mx-auto sm:mx-0 mb-2 bg-brand-100 text-brand-700 inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest">Academic Identity</div>
        ...
        <div className="xl:col-span-2 space-y-6 min-w-0">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-7 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input label="Student ID" value={formData.student_id} onChange={(e) => setFormData({ ...formData, student_id: e.target.value })} />
              <Input label="GPA" type="number" step="0.01" value={formData.gpa} onChange={(e) => setFormData({ ...formData, gpa: e.target.value })} />
            </div>
        ...
        <div className="xl:col-span-1 space-y-6 min-w-0">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-7 min-w-0">
            <div className="border-b border-gray-100 pb-4 mb-5">
              ...
              <p className="text-[10px] text-gray-400 uppercase tracking-widest mt-1 font-bold">PDF · Max 5MB</p>
            </div>
                ...
                return (
                  <div key={bucket} className="relative min-w-0">
                    <label className={`flex items-center gap-3 w-full px-4 py-3.5 border-2 border-dashed rounded-xl transition-all min-w-0 ${
                      url ? "border-green-200 bg-green-50/60 cursor-default" : "border-gray-200 bg-gray-50 hover:bg-gray-100 cursor-pointer"
                    }`}>
                      ...
                      <div className="flex-1 min-w-0 overflow-hidden">
                        ...
                        <p className="text-[10px] text-gray-400 font-medium mt-0.5 text-xs">PDF · Max 5MB</p>
                      </div>
                    </label>
                    {url && (
                      <button type="button" onClick={() => handleDeleteDoc(bucket)} className="absolute -top-2 -right-2 bg-red-500 text-white p-1.5 rounded-full shadow-lg hover:bg-red-600 z-10 border-2 border-white cursor-pointer">
          ...
          <div className="p-5 bg-brand-900 rounded-2xl text-white">
            <h4 className="font-bold mb-1.5 flex items-center gap-2 text-brand-400 text-xs"><Target size={14} /> PRO TIP</h4>
            <p className="text-[11px] text-brand-100/80 leading-relaxed font-semibold">Verified transcripts and a GPA above 3.0 significantly increase your matching engine priority.</p>
          </div>
        </div>
      </form>
    </div>
